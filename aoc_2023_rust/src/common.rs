...
impl Position {
    ...
    pub fn neightbours(&self, diag: bool) -> Vec<Position> {
        let mut ns = Vec::new();
        ns.push(Position::new(self.row - 1, self.col));
        if diag { ns.push(Position::new(self.row - 1, self.col + 1)) };
        ns.push(Position::new(self.row, self.col + 1));
        if diag { ns.push(Position::new(self.row + 1, self.col + 1)) };
        ns.push(Position::new(self.row + 1, self.col));
        if diag { ns.push(Position::new(self.row + 1, self.col - 1)) };
        ns.push(Position::new(self.row, self.col - 1));
